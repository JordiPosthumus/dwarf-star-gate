...
const genie={enabled:true,busy:false,source:'primary',memory,
  status(){return {configured:true,enabled:this.enabled,busy:false,source:this.source,fallback_available:true,last_served_by:'dedicated',mode:'bounded-recovery',predictor_supervision:true,last_check:now-60000,memory:memory?{...memory.status(),...memory.retrieve(snapshot)}:null,
    hardening_notes:[{candidate_id:'f'.repeat(24),title:'Exercise incomplete-stream continuation',suggestion:'Synthetic example: add a deterministic same-socket continuation regression before changing retry policy.',failure_class:'request_failure',scope:'sparkB',reason:'incomplete_sse',observed_at:new Date(now-90000).toISOString(),continuity:'unknown',at:now-60000,revision:1,durable:true}],
