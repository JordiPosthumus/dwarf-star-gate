...
def get_max_tokens(
    ...
    if max_model_len <= input_length:
        raise ValueError(
            f"Input length ({input_length}) leaves no room to generate within "
            f"the model's maximum context length ({max_model_len})."
        )
    ...
    if (
        fallback_max_tokens is not None
        and fallback_max_tokens > effective_max_tokens
        and effective_max_tokens == model_max_tokens
    ):
        logger.info(
            "Output allowance fitted to remaining context: requested=%s "
            "effective=%s prompt=%s context=%s",
            fallback_max_tokens, effective_max_tokens, input_length, max_model_len,
        )
