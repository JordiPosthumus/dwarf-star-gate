...
class TokenizeParams:
    ...
    - `-1` reserves the requested output space before padding.
    """
    ...
    - `-1` reserves the requested output space before truncating.
    """
    ...
    def max_input_tokens(self) -> int | None:
        """Validate the complete prompt independently of the output ceiling.

        Generation uses the exact tokenized prompt length in get_max_tokens().
        Reserving the requested output here rejects or truncates valid prompts.
        """
        return self.max_total_tokens

    @property
    def _auto_input_tokens(self) -> int | None:
        """Preserve explicitly requested automatic truncation/padding."""
        if self.max_total_tokens is None:
            return None
        return max(0, self.max_total_tokens - self.max_output_tokens)
    ...
    def with_kwargs(self, **tokenization_kwargs: Any):
        ...
        max_length = tokenization_kwargs.pop("max_length", self._auto_input_tokens)
    ...
    def get_encode_kwargs(self) -> dict[str, Any]:
        ...
        if max_length is not None and max_length < 0:
            max_length = self._auto_input_tokens
        ...
        if self.truncate_prompt_tokens is None:
            if len(text) > max_input_chars:
                raise VLLMValidationError(
                    ...
                    f"{self.max_total_tokens} tokens. Your prompt alone "
    ...
    def _token_padding(self, tokenizer: TokenizerLike | None, tokens: _S) -> _S:
        ...
        if pad_length is not None and pad_length < 0:
            pad_length = self._auto_input_tokens
