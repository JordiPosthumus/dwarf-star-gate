...
1. **Finish the hardening and efficiency pass.** Continue focused, measured
   ...
   The action ledger also preserves receipts with unrepresentable dates and
   retries unchanged evidence after a failed render. Only successful DOM updates
   count as displayed; storage, row selection and action authority are unchanged.
...
4. **Improve forecast evidence before expanding authority.** Separate engine
   ...
   Later disjoint cohorts now include shared-baseline evidence and a severe
...
- **[Priority Lens](priority-lens.md):** source now includes manual controls,
  ...
  classification and explicitly confirmed chat corrections, with a configurable
  one-hour eligible-wait backstop. Model-quality evidence and live rollout remain. Inference never waits on Genie.
