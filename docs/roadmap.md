...
shell access or an SSH requirement. The separately opted-in removed-job helper
and controller path now require matching enrollment and an acknowledged removed-job
canary before automatic use;
...
it does not grant recovery authority. Direct enrolled-helper native capture now
...
gates, automatic-use certification and observation-only reconciliation are now
fixture-tested. Real removed-job validation remains mandatory on each installation;
no live Mac is certified merely by these source changes. The
