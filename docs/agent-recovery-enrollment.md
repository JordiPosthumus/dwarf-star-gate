...
| Removed Mac registration, including a deleted temporary plist | Not supported for automatic restoration yet |
...
- Restart authority, optional stopped-service start, and fleet-wide automatic
  recovery are distinct choices. Default to proposing restart-only, not silently
...
- **Permissions:** restart, stopped-start and automatic policy, each separately.
