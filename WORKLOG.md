...
- **Removed a timing race from routing-shadow CI.** The idle-worker fixture now
  ...
  production timeouts and scheduling are unchanged.
...
## 2026-09-06 — Proactive Resume progress confirmation

Added a separate result review tied to the native receipt and settled generation. Positive advice must cite the task, cue and subsequent work. The candidate client closes automatic admission while persisting that verdict and revokes on racing input; it never infers progress from settlement alone. The bridge can proceed to the next authorized step after confirmed progress, and stops on completion, acknowledgments without work, human decisions or uncertainty. Enrollment discloses the result review and the terminal displays its outcome.

Still-undispatched reviews may observe newly freed compatible capacity within their existing sixty-second ceiling, without queuing inference or retrying a dispatched request. Validation: 39 native receipt/continuation/CLI checks, 24 real-Pi/faux integration checks and 33 portable guards passed; candidate repository and DSG syntax checks passed. Tests include two successive synthetic tool steps, input before transcript append, active tools, lost acknowledgment and a real progress-journal write failure. Real-model semantic accuracy, production installation and outage continuation remain unverified. No live model settings, sessions or services changed.

## 2026-09-06 — Door pre-forward rejection certificates
