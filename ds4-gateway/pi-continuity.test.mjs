...
for(const mode of ['gateway-wait','certified-retries','partial-stream','terminal-without-reason','reason-without-marker','carriage-return','carriage-return-without-marker','redirect'])test(`real Pi agent ${['partial-stream','terminal-without-reason','redirect'].includes(mode)?'does not replay':'survives'} ${mode} between tool execution and continuation`,{skip:!process.env.DSG_PI_ROOT},async t=>{
  ...
  const backend=http.createServer((req,res)=>{
    ...
    let body='';req.on('data',c=>body+=c);req.on('end',()=>{
      ...
      const sseBody=`data: ${JSON.stringify({id:'test',choices:[{index:0,delta,finish_reason:requests===1?'tool_calls':'stop'}],usage:{prompt_tokens:10,completion_tokens:2}})}\n\n${['reason-without-marker','carriage-return-without-marker'].includes(mode)?'':'data: [DONE]\n\n'}`;
      res.end(mode.startsWith('carriage-return')?sseBody.replaceAll('\n','\r'):sseBody);
    });
  });
  ...
  if(mode==='reason-without-marker'||mode.startsWith('carriage-return'))assert.equal(gateway.stats().workers[0].inference_failures,0,'Pi accepted both explicit finishes; do not count them as worker failures');
});
