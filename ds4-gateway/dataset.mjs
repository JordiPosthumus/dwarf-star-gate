...
export function evidence(kind, raw) {
  ...
  if(kind==='finish'){
    ...
    row.usage_observation=['observed','partial','not_reported','json_capture_limit','unsupported_format','invalid_json','unsupported_route','encoded_body'].includes(raw.usage_observation)?raw.usage_observation:null;
    for(const k of ['request_stream','requested_usage'])row[k]=typeof raw[k]==='boolean'?raw[k]:null;
    row.stream_end=['terminal','terminal_without_done','terminal_without_finish_reason','terminal_reason_unobserved','engine_error','clean_eof_no_terminal','partial_sse_event','observation_limited','encoded_unobserved'].includes(raw.stream_end)?raw.stream_end:null;
  }
  if(kind==='finish'&&raw.generation){
    row.generation=Object.fromEntries(['thinking_characters','answer_characters','tool_characters','first_semantic_ms'].map(k=>[k,number(raw.generation[k])]));
    for(const k of ['output_present','observation_complete'])row.generation[k]=typeof raw.generation[k]==='boolean'?raw.generation[k]:null;
  }
