...
export function hermesProvider(config,{directory,review=false,isCapabilityEnabled=()=>true}) {
  ...
  return {
    get info(){return {capabilities_configured:{rebalance:Boolean(config.queue),research:Boolean(config.research),inspection:Boolean(config.inspection),server_changes:Boolean(config.operations),hourglass:Boolean(config.hourglass)},identity:identityStatus(home),runtime_provenance:runtime,gateway_tracking:!review&&config.gateway_tracking===true,engine:'Hermes',model:config.model,mode:'provider',can_act:!review&&Boolean(config.queue)&&isCapabilityEnabled('rebalance'),queue_available:!review&&Boolean(config.queue)&&isCapabilityEnabled('rebalance'),research_available:Boolean(config.research)&&isCapabilityEnabled('research'),inspection_available:Boolean(config.inspection)&&isCapabilityEnabled('inspection'),operations_available:!review&&Boolean(config.operations)&&isCapabilityEnabled('server_changes'),hourglass_available:!review&&Boolean(config.hourglass)&&isCapabilityEnabled('hourglass')};},
