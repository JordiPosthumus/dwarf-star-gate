...
def register_admission(config,emit):
    ...
    for name,description,parameters in [
        ('verify_serving','Run an owner-requested diagnostic inference check on an existing healthy worker. gateway proves one generation through an existing exclusive model route and the Door; cache checks two interleaved cold-to-warm conversations; tools checks a native tool call and follow-up. These generate synthetic requests, never change settings, clear cache, drain, start or stop anything. Uses test-request max_tokens=4096 only, never a production cap. Returns running promptly; read admission_status for this action ID. Do not repeat the action. Shared serving latencies are not isolated benchmark results.',{'type':'object','properties':{'worker':{'type':'string'},'check':{'type':'string','enum':['gateway','cache','tools']},'action_id':{'type':'string'}},'required':['worker','check','action_id'],'additionalProperties':False}),
