...
export async function lifecycle(options,ops) {
  ...
  const missing=options.kinds.filter(kind=>!ops.registered(kind));
  ...
  if(options.command==='start'){
    ...
    await ops.service('start',options.kinds);
...
export function formatResult(result) {
  const lines=[`DSG ${result.components.join(' + ')} ${result.action} verified.`,...Object.entries(result.urls??{}).map(([kind,url])=>`${kind}${result.action==='stop'?' (stopped)':''}: ${url}`)];
