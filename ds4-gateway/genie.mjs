...
export function briefing(snapshot) {
  ...
  return {time:snapshot.time,gateway_at:snapshot.gateway_at ?? snapshot.time,gateway_stale:!!snapshot.gateway_error,context_length:g?.context_length,draining:!!g?.draining,
    ...
    workers:(g?.workers||[]).slice(0,32).map(w=>({id:w.id,healthy:w.is_healthy,paused:w.drained,quarantine:safeQuarantine(w.quarantine),active:w.load,queued:w.queued,active_seconds:w.active_seconds,
      ...
      recovery_evidence:(()=>{const r=recoveryByWorker.get(w.id);return r?{configured:!!r.configured,state:r.state,reason:r.reason??null,inspected_at:r.inspected_at??null,removal:safeNativeRemoval(r.removal)}:null;})(),
    ...
    semantics:['queue_ms and elapsed_ms are milliseconds for past requests, not the current queue age or an ETA; 120000 ms = 2 minutes',
      ...
      'Native removal evidence is a bounded direct-OS diagnostic matched to the retained PID and boot. Report its checked_at time and observed caller, not an invented cause. A caller such as loginwindow does not prove the removal was accidental; launchctl is a stop-intent warning. Empty/incomplete evidence proves no absence of events. It always has authority none: do not bootstrap, reboot or clear reservations from this diagnostic',
...
You can request ONE bounded recovery action, only when recovery.automatic is true and an exact worker_id/evidence_id pair is present in recovery.offers. Include it as recovery_requests:[{"worker_id":"offered ID","evidence_id":"exact offered evidence ID"}], or use an empty array. The independent DSG runner rechecks current service identity and policy. Depending on the separately enrolled evidence, it may restart a currently running fatal instance or start an exact loaded-but-stopped service; either path preserves its configured launch profile and requires real generation/cache verification before readmission. Never invent an offer, command, endpoint or service name. An action request is NOT a completed repair: never claim a start/restart/recovery succeeded without a completed executor receipt in recent_actions. You have no shell, reboot or session migration authority.
