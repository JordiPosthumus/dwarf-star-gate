...
export function briefing(snapshot) {
  ...
  return {time:snapshot.time,gateway_at:snapshot.gateway_at ?? snapshot.time,gateway_stale:!!snapshot.gateway_error,context_length:g?.context_length,draining:!!g?.draining,
    ...
    queue_timeout_ms:g?.queue_timeout_ms??null,request_timeout_ms:g?.request_timeout_ms??null,
    ...
    workers:(g?.workers||[]).slice(0,32).map(w=>({id:w.id,healthy:w.is_healthy,paused:w.drained,quarantine:safeQuarantine(w.quarantine),active:w.load,max_concurrent_requests:w.max_concurrent_requests??1,queued:w.queued,active_seconds:w.active_seconds,
      ...
      maintenance_locks:(w.maintenance_locks??[]).map(({id,name,created_at,review_at,control_channel})=>({id,name,created_at,review_at,control_channel})),
      oldest_queue_seconds:w.oldest_queue_seconds??null,oldest_queue_remaining_seconds:w.oldest_queue_remaining_seconds??null,
      immediately_free:!!w.is_healthy && !w.drained && !w.quarantine && !g.draining && Number.isSafeInteger(w.load) && w.load>=0 && w.load<(w.max_concurrent_requests??1) && w.queued===0,
      context_length:w.context_length,requested_thinking:w.requested_thinking,
      management_path:w.management_path??null,
      recovery_evidence:(()=>{const r=recoveryByWorker.get(w.id);return r?{configured:!!r.configured,adapter:r.adapter??null,state:r.state,reason:r.reason??null,inspected_at:r.inspected_at??null,removal:safeNativeRemoval(r.removal),...(r.bootstrap?{bootstrap:{enrolled:r.bootstrap.enrolled===true,certified:r.bootstrap.certified===true}}:{})}:null;})(),
    ...
    semantics:['queue_ms and elapsed_ms are milliseconds for past requests, not the current queue age or an ETA; 120000 ms = 2 minutes',
      ...
      'A Door failure_id identifies only its local proxy-failure receipt. When an error response could be sent before headers, it matches that response\'s X-Request-Id/error.continuity.request_id; it is not a backend execution or harness session ID. Missing IDs from older evidence stay unknown. Correlation never grants replay permission, and after-header failures still abort without appending an ID or fabricated completion',
      ...
      'Only continuity_door.model_discovery_hold=true confirms this running Door also holds GET model discovery during core maintenance; older Doors may lack it. held then includes discovery as well as unread body streams. Status/health reads still pass through and may fail during core downtime; local continuity status remains available. failure_evidence counters and its latest 30 receipts cover this Door process only, not durable session history. Missing evidence cannot classify an older failed total. A status transport failure is not a lost inference session or a DS4 fault. before_response_headers and after_response_headers describe the local proxy response, never proof of backend execution: backend_dispatch remains unknown and neither grants replay permission',
      ...
      'Predictor future known_sessions counts identified sessions; unknown_identity_requests cannot satisfy a diversity gate. Legacy sessions is an accounting-group count that may include an unknown placeholder, not proof of independent sessions. No unseen-session support or generalization is established by unidentified requests',
      ...
      'maintenance_locks identify intentional maintenance or test reservations. Report the named reservation alongside unavailable health; a failed model probe during that reservation alone does not establish an unexpected outage or require operator intervention. The lock does not prove native progress, successful restoration or current serving health. Preserve separate failure evidence and never release the lock. review_at is a review reminder, not permission to resume. recovery_evidence.adapter identifies the enrolled recovery mechanism; do not infer a macOS launchd problem for a Docker worker from a generic stopped-service reason',
      'client_watch is an opt-in advisory heartbeat from a client adapter. It contains no prompt, task, tool name, arguments or output. waiting_inside_dsg and model_response_active are correlated with the current gateway process. no_request_reached_dsg means the client reported waiting_for_model but no matching request reached this gateway after the shown threshold; it does not prove a frozen process. client_reported_error means the client reports a settled failed turn after its automatic continuations, not healthy idle. Gateway transport complete can coexist with client protocol failure. Neither observation proves the engine stopped, identifies a DS4 defect or authorizes replay. heartbeat_stale_unknown means silence is unknown, never proof of death. You cannot nudge, revive or control the client',
...
export function parseGenieReview(answer, evidence) {
  try {
    ...
    for(const note of hardening)if(!note||!['candidate_id,suggestion,title','candidate_id,expected_result,suggestion,test,title'].includes(Object.keys(note).sort().join(','))||!candidateIds.has(note.candidate_id))throw new Error();
    const hardening_notes=hardening.map(note=>{
      let suggestion=line(note.suggestion,500);
      if(Object.hasOwn(note,'test')){
        suggestion=`Change: ${suggestion}\nTest: ${line(note.test,500)}\nExpected (not yet verified): ${line(note.expected_result,500)}`;
        if(suggestion.length>500||Buffer.byteLength(suggestion)>1000)throw new Error();
      }
      // Keep the existing bounded notebook schema and old-note compatibility.
...
const REVIEW_INSTRUCTIONS = `You are Gate Genie, the fleet observer for Star Gate.
DSG specializes in antirez's DS4 without editing it. Use existing DS4 API/log/service evidence; do not propose a mandatory private server patch or infer unsupported cache facts.
...
You may request ONE exact pre-dispatch relocation copied from continuity.relocation.genie_offers as relocation_requests:[{"request_id":"...","source":"...","destination":"...","evidence_id":"..."}], or []. These offers exist when two or more jobs are waiting on the source, or after a configured wait threshold, while the destination is immediately free and the request remains undispatched. A queue-pressure signal wakes you to assess the imbalance even when no move is eligible; explain the current blocker in that case. The executor revalidates ownership and preserves the client socket/deadline, but cache locality is explicitly unknown. Weigh the queue pressure, current wait/remaining evidence and potential cache benefit when deciding whether to move; queue-pressure offers do not require waiting for the time threshold. Never invent, edit or claim a relocation succeeded before its executor receipt.
...
export class Genie {
  constructor(config, snapshot, {fetchImpl=genieLoopbackFetch,recover=null,rebalance=null,memory=null,providerLedger=null,assignmentLedger=null,poolUrl=null,isTesting=()=>false}={}) {
    ...
    this.isTesting=isTesting;this.chatQuestions=0;this.reviewDone=Promise.resolve();
  ...
  async answerChat(answer,onWait=()=>{}) {
    this.chatQuestions++;
    try {
      if(this.busy){
        const done=this.reviewDone;onWait(this.busyKind);
        // Same named exception as the legacy Ask path: only our routine review.
        if(this.busyKind==='scheduled'){this.preempted=true;this.abort?.abort();}
        await done;
      }
      if(this.closed)throw new Error('Dashboard stopped before answering');
      return await answer();
    } finally {this.chatQuestions--;}
  }
  async runSubmitted(){
  ...
  async modelAnswer(endpoint,{question,data,history,servedBy='dedicated',flexible=false}) {
    ...
    try {
      response=await this.fetch(`${endpoint.url.replace(/\/$/,'')}/chat/completions`,{method:'POST',redirect:'error',signal:attempt.signal,timeoutMs,
        ...
        body:JSON.stringify({model:endpoint.model||'PoolModel',stream:false,max_tokens:8192,...(endpoint.reasoning_effort===null?{}:{reasoning_effort:endpoint.reasoning_effort??'high'}),
  ...
  async ask(question='Review the current fleet. Flag only evidence-backed issues; distinguish unknowns.',{kind='manual'}={}) {
    ...
    let finishReview;this.reviewDone=new Promise(resolve=>{finishReview=resolve;});
    try {
    ...
    finally {this.reviewFinishedAt=Date.now();this.attempt=this.reviewFinishedAt;this.busy=false;this.busyKind=null;this.preempted=false;this.abort=null;this.activeProvider=null;this.providerStartedAt=null;this.providerDeadlineAt=null;finishReview();if(this.queuedQuestion)queueMicrotask(()=>this.runSubmitted());}
    return this.status();
  }
  tick(){
    ...
    const snapshot=this.getSnapshot(),offers=[
      ...
      ...(snapshot.gateway?.continuity?.relocation?.genie_enabled?(snapshot.gateway.continuity.relocation.diagnostics?.sources??[]).filter(s=>s.genie_pressure).map(s=>`pressure:${s.source}:${s.request_id}:${s.reason}`):[])
