...
export async function runDashboard(configPath, port) {
  ...
  let sparkSetupWatch=null,mediaStandardWatch=null,mediaImprovementWatch=null,pairPreparationWatch=null,pairEnrollmentWatch=null,pairQualificationWatch=null,omlxEnrollmentWatch=null,omlxQualificationWatch=null,omlxSetupWatch=null;
  ...
  const mediaTools=managementEnabled&&config.media_jobs?.enabled?createMediaTools({promote:input=>workerControl(config.control_socket,'/genie-media-promote',input,{channel:'gate_genie'}),qualify:input=>workerControl(config.control_socket,'/genie-media-qualify',input,{channel:'gate_genie'}),improve:input=>workerControl(config.control_socket,'/genie-media-improvement',input,{channel:'gate_genie'}),audit:input=>workerControl(config.control_socket,'/genie-media-audit',input,{channel:'gate_genie'}),repair:input=>workerControl(config.control_socket,'/genie-media-repair',input,{channel:'gate_genie'}),inspectInputs:input=>workerControl(config.control_socket,'/genie-media-inputs',input,{channel:'gate_genie'}),setup:input=>workerControl(config.control_socket,'/genie-media-setup',input,{channel:'gate_genie'}),resources:createMediaResources(config,{isEnabled:()=>isCapabilityEnabled('inspection')}),read:async()=>{const [media,fleet]=await Promise.all([workerControl(config.control_socket,'/media-jobs'),workerControl(config.control_socket,'/workers')]);return {...media,standard_setup:mediaStandardWatch?.status()??null,improvement_watch:mediaImprovementWatch?.status()??null,fleet:fleet.workers.map(w=>({id:w.id,is_healthy:w.is_healthy,drained:w.drained,load:w.load,queued:w.queued}))};},start:input=>workerControl(config.control_socket,'/genie-media-start',input,{channel:'gate_genie'}),isTesting}):null;
  ...
  const server = createDashboard(snapshot, path.join(here,'ui'), managementEnabled ? {
    ...
    media:async()=>({...await workerControl(config.control_socket,'/media-jobs'),standard_setup:mediaStandardWatch?.status()??null,improvement_watch:mediaImprovementWatch?.status()??null}),
    nativeMedia,
  ...
  mediaImprovementWatch=chat&&mediaTools?new MediaImprovementWatch({filename:path.join(path.dirname(config.state_file),'genie','media-improvement-watch.json'),config,chat,read:()=>mediaTools.tool({action:'status'}),isEnabled:()=>!isTesting()&&isCapabilityEnabled('media')&&isCapabilityEnabled('inspection')&&isCapabilityEnabled('server_changes')}):null;
  ...
  await poll(); endpointTelemetry.poll(); const interval = setInterval(poll, 2000), endpointTimer=setInterval(()=>endpointTelemetry.poll(),2000), historyTimer=setInterval(()=>monitoringHistory.save(activity,endpointTelemetry),10000), genieTimer=setInterval(()=>{genie.tick();chat?.tick();void (async()=>{await mediaStandardWatch?.tick();await mediaImprovementWatch?.tick();await mediaWatch?.tick();await sparkSetupWatch?.tick();await pairPreparationWatch?.tick();await pairEnrollmentWatch?.tick();await pairQualificationWatch?.tick();await omlxEnrollmentWatch?.tick();await omlxQualificationWatch?.tick();await omlxSetupWatch?.tick();})();},10000);
  const close = () => { monitoringHistory.save(activity,endpointTelemetry);endpointTelemetry.close(); closed = true; clearInterval(interval);clearInterval(endpointTimer);clearInterval(historyTimer);clearInterval(genieTimer);mediaStandardWatch?.close();mediaImprovementWatch?.close();mediaWatch?.close();sparkSetupWatch?.close();pairPreparationWatch?.close();pairEnrollmentWatch?.close();pairQualificationWatch?.close();omlxEnrollmentWatch?.close();omlxQualificationWatch?.close();omlxSetupWatch?.close();genie.close();chat?.close();operations?.close();hourglass?.close();hardware.close();stopGenieTunnel(); for (const t of timers) clearTimeout(t); for (const child of children) child.kill(); server.closeAllConnections(); server.close(); process.removeListener('SIGTERM', close); process.removeListener('SIGINT', close); };
