...
export async function runDashboard(configPath, port) {
  ...
  const mediaTools=managementEnabled&&config.media_jobs?.enabled?createMediaTools({promote:input=>workerControl(config.control_socket,'/genie-media-promote',input,{channel:'gate_genie'}),qualify:input=>workerControl(config.control_socket,'/genie-media-qualify',input,{channel:'gate_genie'}),improve:input=>workerControl(config.control_socket,'/genie-media-improvement',input,{channel:'gate_genie'}),audit:input=>workerControl(config.control_socket,'/genie-media-audit',input,{channel:'gate_genie'}),repair:input=>workerControl(config.control_socket,'/genie-media-repair',input,{channel:'gate_genie'}),inspectInputs:input=>workerControl(config.control_socket,'/genie-media-inputs',input,{channel:'gate_genie'}),setup:input=>workerControl(config.control_socket,'/genie-media-setup',input,{channel:'gate_genie'}),resources:createMediaResources(config,{isEnabled:()=>isCapabilityEnabled('inspection')}),read:async()=>{const [media,fleet]=await Promise.all([workerControl(config.control_socket,'/media-jobs'),workerControl(config.control_socket,'/workers')]);return {...media,standard_setup:mediaStandardWatch?.status()??null,fleet:fleet.workers.map(w=>({id:w.id,is_healthy:w.is_healthy,drained:w.drained,load:w.load,queued:w.queued}))};},start:input=>workerControl(config.control_socket,'/genie-media-start',input,{channel:'gate_genie'}),isTesting}):null;
