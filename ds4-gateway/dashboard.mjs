...
export async function runDashboard(configPath, port) {
  ...
  const chat=config.genie_chat?new GenieChat({directory:chatDirectory,getSnapshot:snapshot,isSuspended:isTesting,runQuestion:(answer,onWait)=>genie.answerChat(answer,onWait),provider:hermesProvider(genieChatConfig(config),{directory:chatDirectory})}):null;
