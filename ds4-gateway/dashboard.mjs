...
export async function runDashboard(configPath, port) {
  ...
  let sparkSetupWatch=null,mediaStandardWatch=null,pairPreparationWatch=null,pairEnrollmentWatch=null,pairQualificationWatch=null;
  ...
  const recoveryTools=managementEnabled?createRecoveryTools({read:()=>readService('gateway',config),recover:input=>workerControl(config.control_socket,'/genie-recover-worker',input,{channel:'gate_genie'}),
    qualify:input=>workerControl(config.control_socket,'/qualify-pair-recovery',input,{channel:'gate_genie'}),preparation:pairPreparation,enroll:input=>workerControl(config.control_socket,'/enroll-pair-recovery',input,{channel:'gate_genie'}),isChangesEnabled:()=>isCapabilityEnabled('server_changes'),continuation:()=>pairPreparationWatch?.status()??null,enrollmentContinuation:()=>pairEnrollmentWatch?.status()??null,qualificationContinuation:()=>pairQualificationWatch?.status()??null,
  ...
  await poll(); endpointTelemetry.poll(); const interval = setInterval(poll, 2000), endpointTimer=setInterval(()=>endpointTelemetry.poll(),2000), historyTimer=setInterval(()=>monitoringHistory.save(activity,endpointTelemetry),10000), genieTimer=setInterval(()=>{genie.tick();chat?.tick();void (async()=>{await mediaStandardWatch?.tick();await mediaWatch?.tick();await sparkSetupWatch?.tick();await pairPreparationWatch?.tick();await pairEnrollmentWatch?.tick();await pairQualificationWatch?.tick();})();},10000);
  const close = () => { monitoringHistory.save(activity,endpointTelemetry);endpointTelemetry.close(); closed = true; clearInterval(interval);clearInterval(endpointTimer);clearInterval(historyTimer);clearInterval(genieTimer);mediaStandardWatch?.close();mediaWatch?.close();sparkSetupWatch?.close();pairPreparationWatch?.close();pairEnrollmentWatch?.close();pairQualificationWatch?.close();genie.close();chat?.close();operations?.close();hourglass?.close();hardware.close();stopGenieTunnel(); for (const t of timers) clearTimeout(t); for (const child of children) child.kill(); server.closeAllConnections(); server.close(); process.removeListener('SIGTERM', close); process.removeListener('SIGINT', close); };
