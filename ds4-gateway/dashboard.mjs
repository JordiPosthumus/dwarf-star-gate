...
export async function runDashboard(configPath, port) {
  ...
  const mediaTools=managementEnabled&&config.media_jobs?.enabled?createMediaTools({resources:createMediaResources(config,{isEnabled:()=>isCapabilityEnabled('inspection')}),read:async()=>{const [media,fleet]=await Promise.all([workerControl(config.control_socket,'/media-jobs'),workerControl(config.control_socket,'/workers')]);return {...media,fleet:fleet.workers.map(w=>({id:w.id,is_healthy:w.is_healthy,drained:w.drained,load:w.load,queued:w.queued}))};},start:input=>workerControl(config.control_socket,'/genie-media-start',input,{channel:'gate_genie'}),isTesting}):null;
  ...
  const server = createDashboard(snapshot, path.join(here,'ui'), managementEnabled ? {
    ...
    act:async(action,input)=>{if(action==='media-inspect'){if(!mediaTools)throw Error('Media inspection is not connected.');return mediaTools.tool({...input,action:'inspect'});}const value=await workerControl(config.control_socket,({'media-eligibility':'/media-host-eligibility','job-priority':'/set-job-priority',concurrency:'/set-worker-concurrency',add:'/add-worker',endpoint:'/edit-endpoint',test:'/check-endpoint',remove:'/remove-worker',drain:'/drain-workers',resume:'/resume-workers',lock:'/maintenance-lock',unlock:'/release-maintenance-lock',fallbacks:'/set-ssh-fallbacks',context:'/set-context-limit','conversation-turns':'/set-conversation-turns','queue-timeout':'/set-queue-timeout',protection:'/set-protection',relocate:'/relocate-queued',recover:'/recover-worker','genie-capability':'/genie-capability','recovery-policy':'/recovery-policy','recovery-handback-policy':'/recovery-handback-policy','recovery-recheck':'/recovery-recheck'})[action],input,{channel:'dashboard'});if(action==='genie-capability'&&gateway)gateway.genie_capabilities=value;return value;},
  ...
  }:null,chat,hourglass,operations,queueTools,recoveryTools,mediaTools,sparkSetup);
  ...
  sparkSetupWatch=chat&&sparkSetup?new SparkSetupWatch({filename:path.join(path.dirname(config.state_file),'genie','spark-setup-requests.json'),targets:config.spark_setup.targets??{},chat,read:()=>sparkSetup.tool({action:'status'}),isEnabled:()=>!isTesting()&&gateway?.genie_capabilities?.spark_setup===true}):null;
