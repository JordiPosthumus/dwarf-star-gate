...
export function createSparkMediaQualification({directory,transport,fleet,launchRunner=launch}){
 const read=id=>{
 ...
 return {read,async start(id,target){
  ...
  const workers=await fleet();if(!workers.some(w=>w.is_healthy&&!w.drained&&!w.recovering&&!w.quarantine&&!w.maintenance_locks?.length))throw Error('Keep at least one serving gateway LLM during new-host media qualification.');
