...
export function continuityForDisplay(value){
  ...
  return {schema:1,safe_retry_contract:value.safe_retry_contract===true,queued_relocation:false,recent_rejections:(Array.isArray(value.recent_rejections)?value.recent_rejections:[]).slice(0,20).filter(r=>validCallId(r?.request_id)&&rejectionReasons.has(r.reason)&&Number.isFinite(Date.parse(r.time))).map(r=>({time:r.time,request_id:r.request_id,node:typeof r.node==='string'&&/^[\w-]{1,64}$/.test(r.node)?r.node:null,reason:r.reason,dispatch_state:'not_dispatched',retry_class:r.reason==='affinity_write_failed'?'operator_required':'wait_then_retry'}))};
}
