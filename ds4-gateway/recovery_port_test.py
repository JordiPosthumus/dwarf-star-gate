        ...
        for kind in ("launchd", "systemd"):
            ...
            with self.subTest(adapter=kind), socket.socket() as listener, tempfile.TemporaryDirectory() as temp:
                ...
                with patch.object(adapter, "service_profile", return_value="b" * 64):
                    if kind == "launchd":
                        with patch.object(adapter, "boot_identity", return_value=None), patch.object(adapter, "native_disabled", return_value=False), patch.object(adapter, "machine_identity", return_value="a" * 64), patch.object(adapter, "launch_state", return_value={"loaded": True, "stopped": True, "active": False, "pid": 0}), patch.object(adapter, "run") as command:
