...
NAMES = {'read_server_configuration', 'inspect_server'}
...
def read_json(file):
    ...
    try:
        ...
        with os.fdopen(fd,'r',closefd=False) as stream:return json.loads(stream.read(st.st_size))
    finally:os.close(fd)
...
def register_inspection(config, context, emit):
    ...
    def run(kind,args):
        ...
        try:
            ...
            message='Read-only inspection unavailable. No server changes were made; ask the operator to check the configured record or SSH target.'
    ...
    for name,kind,description in [('read_server_configuration','records','Read the full private recorded configuration, launch recipe and artifact references for a configured worker. Dated records are not live evidence. Never publish private fields.'),('inspect_server','live','Inspect the configured worker container and launcher now using a fixed read-only collector. No service changes. Compare with its records; report missing evidence instead of guessing. Currently configured Docker workers only.')]:
        registry.register(name=name,toolset=TOOLSET,schema={'name':name,'description':description,'parameters':{'type':'object','properties':{'worker_id':{'type':'string'}},'required':['worker_id'],'additionalProperties':False}},handler=lambda args,_kind=kind,**kw:run(_kind,args),max_result_size_chars=512000)
    return NAMES
