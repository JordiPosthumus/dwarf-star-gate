...
export function recoveryEvidence(status){
  ...
  return {observed_at:new Date().toISOString(),...recovery,
    ...
    scope:'Current enrolled-service eligibility and existing recovery receipts. configured and adapter describe a registered definition, not a working connection. A mismatched or absent binding is NOT connected. The switch alone does not connect a service. A queued receipt is acceptance, not successful recovery. Explicitly opted-in pairs may be enrolled from native capture receipts. Enrollment does not qualify a restart. For existing local oMLX bindings, explicitly configured start_on_demand policy exposes demand_start_offers; the same enrollment tool can create a new stopped-start wrapper while retaining the original, followed by fresh qualification. Explicitly opted-in idle healthy pairs and enrolled local GLM/oMLX workers offer separate evidence-bound restart qualification; it requires native GLM proof and preserves operator pauses. No arbitrary server configuration changes are available here.'};
}
export function createRecoveryTools({read,recover,preparation=null,enroll=null,enrollOmlx=null,qualifyOmlx=null,qualify=null,isChangesEnabled=()=>false,continuation=()=>null,enrollmentContinuation=()=>null,qualificationContinuation=()=>null,omlxEnrollmentContinuation=()=>null,omlxQualificationContinuation=()=>null,omlxSetupContinuation=()=>null,isInspectionEnabled=()=>true,isTesting=()=>false,isEnabled=()=>true}){
  async function tool(input){
    if(input?.action==='status'&&Object.keys(input).length===1)return {...recoveryEvidence(await read()),...(qualifyOmlx?{omlx_qualification_followup:omlxQualificationContinuation(),omlx_setup_followup:omlxSetupContinuation()}:{}),...(enrollOmlx?{omlx_enrollment_followup:omlxEnrollmentContinuation()}:{}),...(preparation?{pair_preparations:await preparation.status(),pair_preparation_followup:continuation(),pair_enrollment_followup:enrollmentContinuation(),pair_qualification_followup:qualificationContinuation()}: {})};
    ...
    if(input?.action==='qualify-omlx'){
      if(!qualifyOmlx||isTesting()||!isEnabled()||!isInspectionEnabled()||!isChangesEnabled())throw Error('Local oMLX qualification is unavailable or suspended.');
      if(Object.keys(input).sort().join(',')!=='action,action_id,evidence_id,worker_id'||typeof input.worker_id!=='string'||!/^[a-f0-9]{64}$/.test(input.evidence_id??'')||!/^[a-f0-9-]{36}$/.test(input.action_id??''))throw Error('Specify current oMLX qualification evidence and one action ID');
      const receipt=await qualifyOmlx({worker_id:input.worker_id,evidence_id:input.evidence_id,action_id:input.action_id});
      if(receipt?.id!==input.action_id||receipt.worker_id!==input.worker_id||receipt.actor!=='genie'||receipt.omlx_qualification!==true)throw Error('Qualification acknowledgement is uncertain; inspect the same action ID without replaying.');
      return receipt;
    }
