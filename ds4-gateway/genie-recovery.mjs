...
export function createRecoveryTools({read,recover,preparation=null,enroll=null,enrollOmlx=null,qualifyOmlx=null,qualify=null,isChangesEnabled=()=>false,continuation=()=>null,enrollmentContinuation=()=>null,qualificationContinuation=()=>null,omlxEnrollmentContinuation=()=>null,omlxQualificationContinuation=()=>null,isInspectionEnabled=()=>true,isTesting=()=>false,isEnabled=()=>true}){
  async function tool(input){
    if(input?.action==='status'&&Object.keys(input).length===1)return {...recoveryEvidence(await read()),...(qualifyOmlx?{omlx_qualification_followup:omlxQualificationContinuation()}:{}),...(enrollOmlx?{omlx_enrollment_followup:omlxEnrollmentContinuation()}:{}),...(preparation?{pair_preparations:await preparation.status(),pair_preparation_followup:continuation(),pair_enrollment_followup:enrollmentContinuation(),pair_qualification_followup:qualificationContinuation()}: {})};
