...
def register_recovery(config,emit):
    ...
    for name,description,parameters in [
        ('enroll_omlx_recovery','Capture and enroll one explicitly opted-in existing local GLM/oMLX installation. Uses only its configured launcher and dependencies, preserves native process/settings, and returns one durable action ID. Read recovery_status.omlx_enrollment.operations; enrollment does not restart or qualify recovery. For an already enrolled worker, call only when its demand_start_offers entry is eligible and the original task requests on-demand operation. This separately enabled upgrade preserves the original wrapper and settings, adds stopped-start authority in a new binding and requires fresh qualify_omlx_recovery proof. Never replay uncertain work.',{'type':'object','properties':{'worker_id':{'type':'string'}},'required':['worker_id'],'additionalProperties':False}),
