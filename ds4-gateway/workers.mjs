...
try {
  ...
  if(command==='list')route='/workers';
    ...
    route='/add-worker';body={worker:{id,url,...(ssh?{ssh}:{}),...(remote?{remote_port:Number(remote)}:{}),...(journal?{telemetry_service:journal}:{})}};
  ...
  } else throw new Error('Usage: workers.mjs [--config FILE] list|add ID --url URL [--ssh HOST --remote-port PORT]|drain ID|resume ID|remove ID');
