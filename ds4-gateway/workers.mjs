...
try {
  ...
  const backend=option('--backend'), context=option('--context-length'), keyFile=option('--api-key-file');
  ...
  if(command==='list')route='/workers';
    ...
    route='/add-worker';body={worker:{id,url,...(backend?{backend}:{}),...(context?{context_length:Number(context)}:{}),...(keyFile?{api_key_file:keyFile}:{}),...(ssh?{ssh}:{}),...(fallback?{ssh_fallbacks:fallback.split(',').map(v=>v.trim()).filter(Boolean)}:{}),...(remote?{remote_port:Number(remote)}:{}),...(journal?{telemetry_service:journal}:{})}};
  ...
  } else throw new Error('Usage: workers.mjs [--config FILE] list|add ID --url URL [--backend openai --context-length TOKENS --api-key-file FILE] [--ssh HOST --ssh-fallbacks HOST,HOST --remote-port PORT]|fallbacks ID --ssh-fallbacks HOST,HOST|clear-fallbacks ID|drain ID|resume ID|remove ID|lock ID --name NAME --reason REASON [--review-after-hours HOURS] [--request-id UUID]|unlock LOCK_ID --reason REASON [--request-id UUID]|maintenance-receipt REQUEST_ID');
