...
export function createNativeMediaStatus(config,{run=exec,now=Date.now}={}){
  let rows=[],pending=null,last=-Infinity;
  const targets=Object.entries(config.media_jobs?.workers??{}).flatMap(([worker_id,value])=>{
    const host=config.genie_chat?.inspection?.workers?.[worker_id]?.ssh?.[0];
    return Object.entries(value.engines??{}).map(([kind,engine])=>({worker_id,kind,engine:engine.kind,port:engine.port,host}));
  });
  const observe=async target=>{
    const base={worker_id:target.worker_id,kind:target.kind,engine:target.engine};
  ...
  return ()=>{
    if(!pending&&now()-last>=10000){last=now();pending=Promise.all(targets.map(observe)).then(value=>{rows=value;}).finally(()=>{pending=null;});}
    return rows.length?rows:targets.map(t=>({worker_id:t.worker_id,kind:t.kind,engine:t.engine,state:'unknown',reason:'Awaiting native observation',observed_at:null}));
  };
}
