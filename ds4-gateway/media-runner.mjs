...
const nativeIo=(target,prefix='',member=p.llm_pair?.media_member??0)=>{
  ...
  return {
    ...
    connect:async()=>{
      ...
      const child=spawn('ssh',['-N','-o','BatchMode=yes','-o','ExitOnForwardFailure=yes','-o','ServerAliveInterval=15','-o','ServerAliveCountMax=3','-L',`127.0.0.1:${port}:127.0.0.1:${target.engine.port}`,target.host],{stdio:['ignore','ignore',fd]});fs.closeSync(fd);
      await new Promise((resolve,reject)=>{child.once('spawn',resolve);child.once('error',reject);});
      return {backend:new MediaBackend({kind:target.engine.kind,url:`http://127.0.0.1:${port}`}),close:()=>child.kill('SIGTERM')};
    },
  };
};
try{
  ...
  save('completion.json',{...result,...(qualification?{qualification:qualification.completion()}: {})});
  if(qualification)await workerControl(p.control_socket,'/media-qualification-complete',{operation_id:p.ace_qualification.candidate_operation_id});
