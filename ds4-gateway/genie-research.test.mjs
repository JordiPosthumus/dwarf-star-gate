...
test('installed Hermes uses only the two research tools and records actual search/read evidence',{
 ...
 let now=Date.now();const chat=new GenieChat({directory:path.join(d,'chats'),provider,now:()=>now,getSnapshot:()=>({gateway:{workers:[{id:'private-worker'}]}})}),s=chat.create();chat.submit(s.id,'Research the example release','research-hermes');await chat.idle();
 ...
 expectCorrection=true;chat.study.change({action:'study-schedule',expected_revision:chat.study.status().revision,interval_days:1,mode:'automatic'});now+=86400000;chat.tick();chat.tick();await chat.idle();assert.equal(chat.study.status().last_run.state,'complete');assert.equal(searches,4);assert.equal(reads,4);assert.equal(chat.study.status().last_run.evidence.pages_read[0],'https://example.org/release');
});
