...
test('automatic queue wakeup uses pinned Hermes to start once and retain actual tool events',{skip:!process.env.DSG_TEST_HERMES_SOURCE,timeout:120000},async t=>{
  ...
  const server=http.createServer((req,res)=>{
    ...
    let raw='';req.on('data',c=>raw+=c);req.on('end',()=>{
      ...
      if(calls===3)assert.match(JSON.stringify(body.messages),/waiting_idle/);
      const name=calls===2?'start_media_job':'media_job_status',args=calls===2?{job_id:id,worker_id:'one'}:{};
      const message=calls<=3?{role:'assistant',content:null,tool_calls:[{id:'media-'+calls,type:'function',function:{name:'tool_call',arguments:JSON.stringify({name,arguments:args})}}]}:{role:'assistant',content:'Job accepted on one; admitted LLM work is finishing. Generation has not started.'};
