...
test('automatic queue wakeup uses pinned Hermes to start once and retain actual tool events',{skip:!process.env.DSG_TEST_HERMES_SOURCE,timeout:120000},async t=>{
  const directory=fs.mkdtempSync(path.join(os.tmpdir(),'sg-media-chat-'));let starts=0,calls=0,setups=0,inputChecks=0,repairs=0,audits=0,improvements=0,qualifications=0;
  ...
  const server=http.createServer((req,res)=>{
    ...
    let raw='';req.on('data',c=>raw+=c);req.on('end',()=>{
      ...
      const name=calls===2?'inspect_media_host':calls===3?'inspect_media_inputs':calls===4?'start_media_job':calls===6?'setup_media_host':calls===8?'repair_media_setup':calls===10?'audit_media_standard':calls===11?'prepare_media_improvement':calls===12?'qualify_media_improvement':'media_job_status',args=calls===2?{worker_id:'one'}:calls===3?{job_id:id,worker_id:'one'}:calls===4?{job_id:id,worker_id:'one',following_job_ids:[secondId],parallel_members:true}:calls===6?{worker_id:'one',engine:'ace-step'}:calls===8?{worker_id:'one',engine:'ace-step',expected_failed_at:'2026-01-01T00:00:00Z'}:calls===11?{worker_id:'one',member:1}:calls===12?{operation_id:id}:{};
      const message=calls<=12?{role:'assistant',content:null,tool_calls:[{id:'media-'+calls,type:'function',function:{name:'tool_call',arguments:JSON.stringify({name,arguments:args})}}]}:{role:'assistant',content:'Job accepted on one; admitted LLM work is finishing. Generation has not started.'};
      const delta={...message,...(message.tool_calls?{tool_calls:message.tool_calls.map((v,index)=>({...v,index}))}:{})};
      res.setHeader('content-type','text/event-stream');res.end('data: '+JSON.stringify({id:'fixture',model:'fixture',choices:[{index:0,delta,finish_reason:null}]})+'\n\ndata: '+JSON.stringify({id:'fixture',model:'fixture',choices:[{index:0,delta:{},finish_reason:calls<=12?'tool_calls':'stop'}]})+'\n\ndata: [DONE]\n\n');
    });
  });
  ...
  const answer=chat.get(conversation.id).messages[1];assert.equal(answer.state,'complete',JSON.stringify(answer));assert.equal(starts,1);assert.equal(calls,13);assert.equal(qualifications,1);assert.equal(improvements,1);assert.equal(audits,1);assert.equal(repairs,1);assert.equal(inputChecks,1);assert.equal(setups,1);assert.equal(provider.info.can_act,true);
  assert.equal(answer.media.events.filter(e=>e.state==='complete').length,12);assert.ok(answer.media.events.some(e=>e.tool==='inspect_media_inputs'&&e.state==='complete'));assert.ok(answer.media.events.some(e=>e.tool==='start_media_job'&&e.state==='complete'));assert.ok(answer.media.events.some(e=>e.tool==='repair_media_setup'&&e.state==='complete'));assert.ok(answer.media.events.some(e=>e.tool==='audit_media_standard'&&e.state==='complete'));assert.equal(chat.capabilityActivity().media.state,'complete');
