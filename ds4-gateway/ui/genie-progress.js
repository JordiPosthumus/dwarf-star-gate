...
export function chatProgress(message,{now=Date.now(),connected=true,suspended=false}={}){
  ...
  if(latest&&['records','live','artifact'].includes(latest.kind)&&latestAt>=(p?.at??0))label=latest.state==='reading'?`Inspecting ${latest.worker_id}`:latest.state==='complete'?`Inspection returned · ${latest.worker_id}`:`Inspection unavailable · ${latest.worker_id}`;
