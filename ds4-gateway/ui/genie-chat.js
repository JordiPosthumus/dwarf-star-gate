...
if(panel){
  ...
  function render(session){
    ...
    for(const m of session?.messages??[]){
      ...
      if(m.measurements?.events?.length){const d=text('details');d.append(text('summary','Hourglass tool calls'));for(const e of m.measurements.events){d.append(text('p',`${e.tool} · ${e.state}`));if(e.request)d.append(text('pre',JSON.stringify(e.request,null,2)));if(e.result)d.append(text('pre',JSON.stringify(e.result,null,2)));}article.append(d);}
      if(m.operations?.events?.length){const d=text('details');d.append(text('summary','Server change tool calls'));for(const e of m.operations.events){d.append(text('p',`${e.tool} · ${e.state} · ${e.id??'list'}`));if(e.result)d.append(text('pre',JSON.stringify(e.result,null,2)));}article.append(d);}
      if(m.inspection?.events?.length){const d=text('details','','conversation-inspection');d.append(text('summary','Server inspection evidence'));for(const e of m.inspection.events){d.append(text('p',`${e.worker_id} · ${e.kind} · ${e.state} · ${new Date(e.finished_at??e.at).toLocaleString()}`));if(e.revision)d.append(text('p',`Result revision: ${e.revision}`));if(e.error)d.append(text('p',e.error));if(e.result)d.append(text('pre',JSON.stringify(e.result,null,2)));}article.append(d);}
      if(m.recovery?.events?.length){const d=document.createElement('details');d.open=m.state==='working';d.append(text('summary','Server recovery activity'));for(const e of m.recovery.events){d.append(text('p',`${e.tool==='recover_server'?'Request recovery':'Read recovery status'} · ${e.state}`));if(e.action_id)d.append(text('p',`Operation: ${e.action_id}`));if(e.error)d.append(text('p',e.error));if(e.result)d.append(text('pre',JSON.stringify(e.result,null,2)));}article.append(d);}
      if(m.spark_setup?.events?.length){const d=document.createElement('details');d.open=m.state==='working';d.append(text('summary','New Spark setup activity'));for(const e of m.spark_setup.events){d.append(text('p',`${e.tool} · ${e.state}`));if(e.error)d.append(text('p',e.error));if(e.result)d.append(text('pre',JSON.stringify(e.result,null,2)));}article.append(d);}
      if(m.media?.events?.length){const d=document.createElement('details');d.open=m.state==='working';d.append(text('summary','Media activity'));for(const e of m.media.events){d.append(text('p',`${e.tool==='setup_media_host'?'Set up media engine':e.tool==='start_media_job'?'Start media job':e.tool==='inspect_media_inputs'?'Check media input files':e.tool==='inspect_media_host'?'Inspect media resources':'Read media status'} · ${e.state}`));if(e.error)d.append(text('p',e.error));if(e.result)d.append(text('pre',JSON.stringify(e.result,null,2)));}article.append(d);}
      if(m.queue?.events?.length){const d=document.createElement('details');d.open=m.state==='working';d.append(text('summary','Queue balancing activity'));for(const e of m.queue.events){d.append(text('p',`${e.tool==='move_waiting_job'?'Move waiting job':'Read queue'} · ${e.state}`));if(e.error)d.append(text('p',e.error));if(e.result)d.append(text('pre',JSON.stringify(e.result,null,2)));}article.append(d);}
    ...
    $('conversation-web-status').textContent=status?.research_available?'Genie can search public sources when useful.':status?.capabilities_configured?.research?'Public web research is switched off.':'Web search is not configured for this installation.';
  ...
  async function refresh(){if(fetching)return;fetching=true;try{
    ...
    $('conversation-badge').textContent=status.mode==='rehearsal'?'Example setup':status.queue_available&&status.recovery_available?'Balancing and recovery enabled':status.queue_available?'Queue balancing enabled':status.recovery_available?'Recovery enabled':status.operations_available?'Changes need approval':'No server changes';
