...
if(panel){
  ...
  async function refresh(){if(fetching)return;fetching=true;try{
    ...
    $('conversation-badge').textContent=status.mode==='rehearsal'?'Example setup':status.queue_available?'Queue balancing enabled':status.operations_available?'Changes need approval':'No server changes';
