...
test('installed Pi serializer supplies exact affinity for optional priority intent without changing model capabilities',{skip:!process.env.DSG_PI_ROOT,timeout:30000},async t=>{
  ...
  const dir=fs.realpathSync(fs.mkdtempSync(path.join(os.tmpdir(),'dsg-pi-priority-'))),requests=[],envelopes=[],errors=[];let session,gateway,tools=0;
  ...
  const model={id:'deepseek-v4-flash',name:'Fixture',reasoning:true,thinkingLevelMap:{xhigh:'xhigh'},input:['text','image'],contextWindow:262144,maxTokens:262144,cost:{input:0,output:0,cacheRead:0,cacheWrite:0},compat:{supportsReasoningEffort:true,sendSessionAffinityHeaders:true}};
  ...
  const loader=new DefaultResourceLoader({cwd:dir,agentDir:dir,settingsManager:settings,noExtensions:true,noSkills:true,noPromptTemplates:true,noThemes:true,noContextFiles:true,systemPrompt:'PRIVATE_SYSTEM_NOT_FOR_CLASSIFIER',extensionFactories:[pi=>{
    registerPiContinuity(pi,{provider,baseUrl,streamSimple,priorityLens:true,priorityFetchImpl:async(url,init)=>{envelopes.push(JSON.parse(init.body));return fetch(url,init);}});
  }]});await loader.reload();
  ...
  for(const request of requests){assert.equal(request.headers['x-session-affinity'],envelopes[0].session);assert.equal(request.headers['x-dsg-priority-intent'],undefined);assert.equal(request.payload.reasoning_effort,'xhigh');}
  assert.ok(!JSON.stringify(envelopes).includes('PRIVATE_'));assert.deepEqual(errors,[]);
  let review;for(let i=0;i<20&&!review;i++){review=(await workerControl(control,'/priority-review-next',{})).review;if(!review)await new Promise(resolve=>setTimeout(resolve,10));}
  assert.equal(review.excerpt,envelopes[0].excerpt);
  ...
  assert.equal((await workerControl(control,'/priority-review-result',{lease:review.lease,intent_id:review.intent_id,advice:{priority:'High',reason:'urgent'}})).accepted,false,'new user input rejects old in-flight classification');
});
