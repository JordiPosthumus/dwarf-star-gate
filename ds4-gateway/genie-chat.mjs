...
export class GenieChat {
  ...
  capabilityActivity() {
    const latest={};
    for(const session of this.sessions.values())for(const message of session.messages)for(const key of ['research','inspection','queue','recovery'])for(const event of message[key]?.events??[]){
      ...
      latest[key==='queue'?'rebalance':key]={at,state:event.state,service:key==='research'?(event.kind==='search'?'Web search':'Page extraction'):key==='queue'?'Queue balancing':key==='recovery'?(event.request?.worker_id??'Server recovery'):event.worker_id,error:event.state==='failed'?event.error:null};
    }
    return latest;
  }
  status() {
