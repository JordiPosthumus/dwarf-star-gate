...
const paths = new Set(['/workers','/current-jobs','/set-job-priority','/set-worker-concurrency', '/add-worker', '/edit-endpoint', '/check-endpoint', '/remove-worker', '/drain-workers', '/resume-workers', '/maintenance-lock','/release-maintenance-lock','/maintenance-receipt','/set-ssh-fallbacks','/set-context-limit','/set-conversation-turns','/set-queue-timeout','/set-protection','/relocate-queued','/genie-relocate-queued','/genie-capability','/recovery-policy','/recovery-handback-policy','/recover-worker','/genie-recover-worker','/recovery-canary','/recovery-recheck','/agents','/grant-agent','/revoke-agent','/release-agent-hold']);
export function workerControl(socketPath, route, body, {channel}={}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({ socketPath, path: route, agent:false, method: ['/workers','/agents','/current-jobs'].includes(route) ? 'GET' : 'POST', headers:{'content-type':'application/json',...(channel?{'x-dsg-control-channel':channel}:{})} }, res => {
      let data = '';
