...
const paths = new Set(['/workers','/priority-status','/priority-settings','/priority-manual','/priority-rules','/priority-review-next','/priority-review-result', '/add-worker', '/remove-worker', '/drain-workers', '/resume-workers', '/maintenance-lock','/release-maintenance-lock','/maintenance-receipt','/set-ssh-fallbacks','/set-context-limit','/set-queue-timeout','/set-protection','/relocate-queued','/genie-relocate-queued','/recovery-policy','/recovery-handback-policy','/recover-worker','/genie-recover-worker','/recovery-canary','/recovery-recheck','/predictor','/genie-predictor','/agents','/grant-agent','/revoke-agent','/release-agent-hold']);
export function workerControl(socketPath, route, body, {channel}={}) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const req = http.request({ socketPath, path: route, agent:false, method: ['/workers','/agents','/priority-status'].includes(route) ? 'GET' : 'POST', headers:{'content-type':'application/json',...(channel?{'x-dsg-control-channel':channel}:{})} }, res => {
      let data = '';
