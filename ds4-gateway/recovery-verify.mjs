...
export async function verifyRecovery(url,model,context,{fetchImpl=fetch,signal,kind='ds4',endpoint}={}) {
  if(!['ds4','qwen_vllm','qwen_omlx'].includes(kind))throw new Error('verification_kind_unsupported');
  ...
  if(models.data?.find(m=>m.id===servedModel)?.[qwen?'max_model_len':'context_length']!==context)throw new Error('verification_context_changed');
  ...
  return {check:qwen?kind+'_two_conversations_cold_to_warm':'two_conversations_cold_to_warm',context_length:context,samples,verified_at:new Date().toISOString()};
}
