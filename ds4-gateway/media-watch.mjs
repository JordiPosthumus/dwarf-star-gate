...
export class MediaWatch {
  ...
  async tick(){
    ...
    try{
      ...
      const candidates=(s.jobs??[]).filter(j=>j.state==='queued'&&!j.execution&&!j.dispatch_hold&&
        !(s.batches??[]).some(b=>b.id===j.batch_id&&b.remaining_requested_slots===0)).sort((a,b)=>priorityRank(b)-priorityRank(a));
      ...
      if(!job)return;
      const film=(s.batches??[]).find(b=>b.id===job.batch_id);
      const key=createHash('sha256').update(JSON.stringify({job:job.id,workers:s.workers,fleet:s.fleet,...(film?{film}: {})})).digest('hex');
