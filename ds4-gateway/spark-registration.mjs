...
import {serviceInputFromProof,sparkServiceBinding} from './spark-services.mjs';
...
export function createSparkRegistration({directory,recordsDirectory,control,port=freePort}){
  ...
  return {read,async register(id,target,proof,media={}){
    ...
    const services=serviceInputFromProof(proof,media);
    if(services){
      if(fleet.spark_services_version!==1)throw new Error('The running gateway needs new-Spark service enrollment support; no worker was added.');
      sparkServiceBinding(worker,services);
    }
    ...
    try{
      ...
      const record={...proof.configuration_evidence,schema:1,worker_id:id,kind:'observed',recorded_at:proof.verified_at,
        ...
        ...(services?{recovery_qualification:{status:'passed',kind:'same_container_restart',helper_sha256:proof.recovery.helper_sha256,receipt_sha256:proof.recovery.restart_receipt_sha256,native_result_sha256:proof.configuration_evidence.native_result_sha256}}:{}),
        scope:services?'Native serving after a same-container recovery restart. Retained-version rollback is separate. Media qualification covers only enrolled kinds.':'Observed new-host qualification. Does not grant automatic recovery or prove media engines.'};
      ...
      const added=await control('/add-worker',{worker,...(services?{services}:{})});
      ...
      row.state='registered_paused';if(services)row.services={recovery:true,media:Object.keys(media)};save(file,row);
