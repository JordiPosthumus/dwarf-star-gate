...
NAMES = {'media_job_status', 'start_media_job'}
...
def register_media(config, emit):
    ...
    def run(name, args):
        ...
        try:
            payload = {'action': 'status'} if name == 'media_job_status' else {'action': 'start', **args}
            ...
            message = 'Media request was not confirmed. Read media_job_status for the same job; do not enqueue a replacement.'
