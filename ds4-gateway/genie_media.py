...
def register_media(config, emit):
    ...
    def run(name, args):
        ...
        try:
            payload = ({'action': 'job', 'job_id': args['job_id']} if 'job_id' in args else {'action': 'overview'}) if name == 'media_job_status' else {'action': 'inputs' if name == 'inspect_media_inputs' else 'inspect' if name == 'inspect_media_host' else 'setup' if name == 'setup_media_host' else 'start', **args}
