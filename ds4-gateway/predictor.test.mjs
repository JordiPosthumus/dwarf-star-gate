...
test('analytics freezes independent model versions/stages and excludes failed and pre-30s remaining estimates',()=>{
  ...
  a.accept(row('finish','a',50002,{outcome:'complete',finish_reason:'stop',service_ms:50000}));let s=a.snapshot().model_series;assert.equal(s.length,2);assert.equal(s[1].rows.length,1);assert.equal(s[1].rows[0].service_ms,20000);
