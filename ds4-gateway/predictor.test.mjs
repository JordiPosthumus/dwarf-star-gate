...
test('analytics freezes independent model versions/stages and excludes failed and pre-30s remaining estimates',()=>{
  ...
  a.accept(row('finish','a',50002,{outcome:'complete',finish_reason:'stop',service_ms:50000}));let s=a.snapshot().model_series;assert.equal(s.length,2);let remaining=s.find(m=>m.stage==='remaining');assert.equal(remaining.rows.length,1);assert.equal(remaining.rows[0].service_ms,20000);assert.equal(remaining.last_forecast_at,origin+30002);
  a.accept(decision('missing',60000));a.accept(row('dispatch','missing',60001,{queue_ms:1}));a.accept(row('finish','missing',100001,{outcome:'complete',finish_reason:'stop',service_ms:40000}));s=a.snapshot().model_series;remaining=s.find(m=>m.stage==='remaining');assert.equal(remaining.rows.length,2);assert.equal(remaining.rows[1].forecast_eligible,true);assert.equal(remaining.rows[1].predicted_service_ms,null);assert.equal(remaining.rows[1].service_ms,null,'missing remaining forecasts must not fabricate an actual remaining label');
  assert.equal(remaining.last_forecast_at,origin+30002,'coverage rows without a forecast cannot make an old model look current');
});
