...
export async function enrollProactiveResume({session,ui,reviewer,taskMessage,gatewayBaseUrl,providers,expiresAt,attemptBudget,createReceipts,signal,start=true,allowUndispatchedOutage=false,outageObservation,outageReady}={}){
  ...
  const prepared=session.prepareContinuationEnrollment({gatewayBaseUrl,allowUndispatchedOutage,enrollment:{scopeId,sessionId,expiresAt,attemptBudget}});
  let receipts,bridge;
  ...
  try{
    ...
    bridge=new ProactiveResumePi({session,control,reviewer,taskMessage,scopeId,outageReady,consent:{reviewText:true,reviewProgress:true,reviewOutage:allowUndispatchedOutage,providers:disclosed}});
