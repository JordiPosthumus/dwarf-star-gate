...
const kinds={
  'omlx-qualification':{tool:'qualify_omlx_recovery',rows:r=>(r?.operations??[]).filter(o=>o.omlx_qualification===true).map(o=>({...o,action_id:o.id})),states:['recovered','verified_paused','failed','reconciliation_needed'],prefix:'omlx-qual-',label:'oMLX restart qualification',limit:'Recovered with native GLM/oMLX proof qualifies this exact existing launcher and profile. Paused, failed or uncertain outcomes do not authorize another restart. Read the terminal receipt without replaying the action, overriding a pause, changing settings or claiming stopped-start authority.'},
  'omlx-enrollment':{tool:'enroll_omlx_recovery',rows:r=>r?.omlx_enrollment?.operations??[],states:['enrolled','failed'],prefix:'omlx-enroll-',label:'oMLX enrollment',limit:'Enrolled means the existing local oMLX binding was captured and installed without a restart. Native restart qualification remains required. After reading the terminal receipt, you may continue the original requested on-demand setup only: use enroll_omlx_recovery for a fresh eligible demand_start_offers entry, or qualify_omlx_recovery for fresh eligible omlx_qualification evidence on the newly installed binding. Never replay an existing action, interrupt admitted work, override an owner pause or change model settings.'},
