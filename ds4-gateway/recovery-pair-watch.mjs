...
const terminal=row=>['prepared','failed'].includes(row?.state);
...
function captures(conversation){
  const latest=new Map(),observed=new Set(),observedActions=new Set();
  for(const message of conversation.messages){
    if(message.role!=='assistant')continue;
    for(const event of message.recovery?.events??[]){
      if(event.tool==='prepare_pair_recovery'&&uuid.test(event.action_id??'')&&typeof event.request?.worker_id==='string'){
      ...
      if(event.tool==='recovery_status'&&event.state==='complete'){
        for(const row of event.result?.pair_preparations??[])if(terminal(row)){observed.add(receiptKey(row));observedActions.add(row.action_id);}
      }
    }
  }
  ...
  async tick(){
    ...
    try{
      const conversations=summary.conversations.filter(c=>!c.queue_paused).map(c=>this.chat.get(c.id)).filter(conversation=>{
        const {latest,observed,observedActions}=captures(conversation);
      ...
      for(const original of conversations){
        ...
        const {latest,observed}=captures(conversation);
        ...
        if(!matched.length||matched.some(r=>!terminal(r)))continue;
        ...
        if(!record){
          ...
          const request_id='pair-result-'+createHash('sha256').update(JSON.stringify([conversation.id,action_ids])).digest('hex');
          record={conversation_id:conversation.id,request_id,action_ids,receipts:needed.map(receiptKey),state:'pending',
            text:`The native pair capture watcher has terminal receipts for these previously requested action IDs: ${action_ids.join(', ')}. Read recovery_status once and report their actual states, worker IDs and evidence hashes in at most 100 words. This is an automatic completion follow-up to the existing captures. Do not start or retry a capture, enroll recovery, run a canary, restart a service or change configuration. Prepared means inspection evidence only; recovery enrollment and restart qualification remain separate. If evidence is unavailable or disagrees, report that uncertainty and finish.`};
