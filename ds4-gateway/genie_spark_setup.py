...
NAMES = {'setup_spark', 'spark_setup_status', 'prepare_spark', 'qualify_spark_llm', 'register_spark_llm'}
...
def register_spark_setup(config, emit):
    ...
    def run(name, args):
        ...
        try:
            payload = {'action': 'status'} if name == 'spark_setup_status' else {'action': 'setup' if name == 'setup_spark' else 'register' if name == 'register_spark_llm' else 'qualify' if name == 'qualify_spark_llm' else 'start', **args}
    ...
    schemas = [
        ('setup_spark', 'Request complete LLM onboarding for an enrolled new Spark: prepare the standard engines, then qualify and register the LLM. A saved request wakes Genie after each long stage, across dashboard restarts. The enabled setup switch grants permission; turning it off pauses new stages while accepted work continues. Does not enroll media or automatic recovery. Use prepare_spark instead for preparation only.', {'type': 'object', 'properties': {'target_id': {'type': 'string'}}, 'required': ['target_id'], 'additionalProperties': False}),
