...
export function createRecipeTrials({config,powerBusy=()=>false,launch=spawn}={}){
  ...
  async function start({profile,stage,trial_id}){
    if(!uuid.test(trial_id??'')||!['prepare','run'].includes(stage)||!Object.hasOwn(enrolled,profile))throw Error('Use an enrolled recipe profile, prepare/run stage and one trial UUID.');
    ...
    if(stage==='prepare')fs.writeFileSync(savedPlan,bytes,{flag:'wx',mode:0o600});
    else{
    ...
    const receipt={trial_id,profile,worker:plan.worker,stage,plan_sha256:binding.plan_sha256,state:'starting',started_at:new Date().toISOString()};
