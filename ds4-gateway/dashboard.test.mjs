...
test('thinking UI distinguishes requested controls, omitted/unknown, current/last and stale values', () => {
  ...
  assert.equal(info({status:'specified',fields:{reasoning_effort:'low'},served}).label,'High');
  ...
  assert.equal(info({status:'specified',fields:{reasoning_effort:'max'},served:{...served,mode:'xhigh'}}).label,'Xhigh');
  assert.equal(info({status:'specified',fields:{thinking:false},served:{...served,mode:'none'}}).label,'Off');
  assert.equal(info({status:'specified',fields:{reasoning_effort:'low'}}).label,'Low');
  assert.equal(info({status:'not_specified',served}).label,'High');
  assert.equal(info({status:'not_specified'}).label,null);
  assert.equal(info({status:'pending'}).label,null);
  assert.equal(info({status:'unavailable',reason:'capture_limit'}).label,null);
  assert.equal(info(null).label,null);
  const worker = {load:1,requested_thinking:{status:'specified',fields:{reasoning_effort:'low'},served},last_requested_thinking:{status:'specified',fields:{reasoning_effort:'high'},served},last_request_finished_at:'2026-09-02T00:00:00Z'};
  const current = vm.runInContext(`thinkingIndicator(${JSON.stringify(worker)},false,1788310000000)`,context);
  assert.match(current,/>High</); assert.doesNotMatch(current,/>Low</); assert.match(current,/Requested: reasoning_effort=low/);
  assert.match(current,/>Thinking:</);assert.doesNotMatch(current,/Requested </);
  assert.doesNotMatch(current,/class="thinking-scope"/,'current load=1 shows no scope chip');
  ...
  assert.match(vm.runInContext(`thinkingIndicator(${JSON.stringify(worker)},true,1788310000000)`,context),/Stale/);
  const gap = {load:1,requested_thinking:null,last_requested_thinking:{status:'specified',fields:{reasoning_effort:'max'}},last_request_finished_at:'2026-09-02T00:00:00Z'};
  const fallback = vm.runInContext(`thinkingIndicator(${JSON.stringify(gap)},false,1788310000000)`,context);
  assert.match(fallback,/>Max</,'unspecified current falls back to the last known level instead of Unavailable'); assert.match(fallback,/>Last</);
...
test('Current Jobs API is read-only, same-origin and separate from diagnostics',async t=>{
  ...
  assert.equal((await fetch(origin+'/api/current-jobs',{headers:{origin:'http://other.example'}})).status,403);
  assert.equal((await fetch(origin+'/api/current-jobs',{method:'POST',headers:{origin},body:'{}'})).status,405);
  assert.equal((await fetch(origin+'/api/priority')).status,404);
