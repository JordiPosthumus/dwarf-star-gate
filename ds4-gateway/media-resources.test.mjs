...
test('media resources keep permission, observations and fit claims separate',async()=>{
  ...
  const [a,b]=await Promise.all([first,second]);assert.deepEqual(a,b);assert.equal(calls,1);assert.equal(a.recipe_platform_matches,true);assert.match(a.setup,/native qualification/);assert.equal(a.recipes.find(r=>r.engine==='h3').model_bytes_required,42470585471);assert.equal(mediaRecipeResources.find(r=>r.engine==='ace-step').model_bytes_required,28496774102);
