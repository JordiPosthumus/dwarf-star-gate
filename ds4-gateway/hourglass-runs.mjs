...
const active=r=>['submitting','uncertain','accepted','pending','running','unknown'].includes(r.state);
...
const states=new Set(['submitting','uncertain','accepted','pending','running','unknown','completed','stopped','error','cancelled','rejected','owner_checked']);
const scope='Owner-started Hourglass measurements. Hourglass owns execution, clock and results. No automatic starts, cancellation, resume, server changes or contention protection.';
...
export class HourglassRuns {
  constructor(config,directory,{client,records=()=>({records:[]}),now=Date.now}={}){
    ...
    this.targets=config.targets.map(t=>{
      if(!t||typeof t.model!=='string'||!t.model.trim()||t.model.length>256||!/^\w[\w-]{0,63}$/.test(t.worker_id)||!['direct','gateway','testing-door'].includes(t.route)||Object.keys(t).some(k=>!['model','worker_id','route'].includes(k)))throw new Error('Invalid Hourglass target association.');
      return {...t};
    });
  ...
  async change(input){
    ...
    const keys={prepare:['action','model'],start:['action','id','owner_confirmed_idle'],refresh:['action'],resolve:['action','id','checked_in_hourglass']}[input?.action];
    ...
    try{
      if(input.action==='prepare'){
        ...
        if(this.runs.some(active)||!this.prepared||input.id!==this.prepared.id||input.owner_confirmed_idle!==true)throw new Error('Review the run and confirm its free measurement window.');
        ...
        const row=this.runs.find(r=>r.id===input.id);if(!row||!needsCheck(row)||input.checked_in_hourglass!==true)throw new Error('Check the uncertain request in Hourglass first.');
  ...
  async refresh(){
    for(const row of this.runs.filter(r=>r.job_id&&r.state!=='owner_checked'&&(!terminal(r)||!r.report))){
      try{
  ...
  startObserving(){this.timer=setInterval(()=>{if(!this.busy&&!this.closed&&!this.error&&this.runs.some(r=>r.job_id&&r.state!=='owner_checked'&&(!terminal(r)||!r.report)))void this.change({action:'refresh'}).catch(()=>{});},15000);this.timer.unref();}
