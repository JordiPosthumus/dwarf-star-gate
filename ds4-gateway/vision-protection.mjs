...
export const IMAGE_LIMIT_GUIDANCE="DSG: This request contains more than DS4's limit of 16 images across the submitted conversation, INCLUDING older turns. One contact-sheet PNG counts as ONE image regardless of tiles. Reading one new image does not remove the old ones. Use your harness's visual-context selection (dsg_visual_context when enrolled), or compact the submitted visual history, then resend the relevant images and continue the original task. No visual assessment was performed by this guidance response; this is a request-repair instruction, not a final answer to the task. (This is a message from the DSG gateway.)";
