...
export function videoCapabilities(status,config){
  ...
  return {schema:1,enabled:status.enabled===true,automatic_dispatch_enabled:status.automatic_dispatch_enabled===true,
    ...
    submission:{formats:['text_prompt','native_workflow'],atomic_film_batches:true,max_batch_clips:128,max_json_bytes:2*1024*1024,
      priorities:['high','normal','idle-only'],idempotency_key_required:true,insufficient_capacity:'queued',requested_parallelism_supported:true,
      requested_parallelism:{field:'requested_parallelism',minimum:1,maximum:128,optional:true,scope:'Per-film ceiling; never overrides owner physical budget or serving availability. Omit to use available policy-controlled capacity.'}},
